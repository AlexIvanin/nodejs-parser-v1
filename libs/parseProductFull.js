...
async function run() {
    ...
    var queueHandled = 0,
        queueTotal = await List.count({images:""}),
    ...
    while(queueHandled < queueTotal) {
        ...
        let limit = parseInt(ENV.PAGE_SIZE),
            offset = queueHandled,
            productList = await List.find({images:""}).skip(offset).limit(limit).exec();
...
function parseProduct(parser) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            let product = await List.find({idProduct: newEntity.id, images:""});
